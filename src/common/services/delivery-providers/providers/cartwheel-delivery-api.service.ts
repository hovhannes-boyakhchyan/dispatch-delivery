...
@Injectable()
export class CartwheelDeliveryApiService implements AbstractDeliveryService {
  private logger: Logger = new Logger(CartwheelDeliveryApiService.name);
  ...
  async getQuote(data: CreateQuoteDto): Promise<ProviderQuotesDto> {
    return firstValueFrom(
      this.httpService
        .post(
          `http://${process.env.CARTWHEEL_DELIVERY_SERVICE_HOSTNAME}:${process.env.CARTWHEEL_DELIVERY_PORT}/quote/create`,
          data,
        )
        .pipe(
          map((response) => {
  ...
  async createDelivery(
    ...
    return firstValueFrom(
      this.httpService
        .post(
          `http://${process.env.CARTWHEEL_DELIVERY_SERVICE_HOSTNAME}:${process.env.CARTWHEEL_DELIVERY_PORT}/job/create`,
          data,
        )
        .pipe(
          map((response) => {
  ...
  async cancelDelivery(deliveryId: string, data?: CancelJobDto): Promise<void> {
    return firstValueFrom(
      this.httpService
        .post(
          `http://${process.env.CARTWHEEL_DELIVERY_SERVICE_HOSTNAME}:${process.env.CARTWHEEL_DELIVERY_PORT}/job/cancel/${deliveryId}`,
          data,
        )
        .pipe(
          map((response) => {
